...
class BacktestingEngine(object):
    ...
    def loadHistoryData(self):
        ...
        for d in initCursor:
            data = dataClass()
            data.__dict__ = d
            #print ''.join('\t').join( [ str(x) for x in d.items()])
            #self.output(u"#1# %s"%(str(d)))
            #self.output(u"#2# %s"%(str(data)))
        ...
        print "最后录入数据数量：", self.dbCursor.count()
